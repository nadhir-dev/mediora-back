...
class DoctorRequest(BASE):
    ...
    request_media: Mapped[List["RequestMedia"]] = relationship(
        back_populates="doctor_request", cascade="all, delete-orphan"
    )
...
class RequestMedia(BASE):
    ...
    media: Mapped["Media"] = relationship(back_populates="request_media", uselist=False)
    doctor_request: Mapped["DoctorRequest"] = relationship(
        back_populates="request_media"
    )
